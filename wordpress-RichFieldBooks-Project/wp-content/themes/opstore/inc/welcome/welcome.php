<?php
	if(!class_exists('Opstore_Welcome')) :

		class Opstore_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' 		=> esc_html__('Getting Started', 'opstore'),
					'recommended_plugins' 	=> esc_html__('Recommended Plugins', 'opstore'),
					'demo_import' 			=> esc_html__('Import Demo', 'opstore'),
					'changelogs'			=> esc_html__('Changelogs','opstore'),
					'free_vs_pro'			=> esc_html__('Free Vs Pro','opstore'),
			...
			<p>
			<?php
			printf( '%1$s %2$s %3$s <a href="%4$s">%5$s</a> %6$s', esc_html__( 'Welcome! Thank you for choosing', 'opstore' ), esc_html($this->theme_name), esc_html__( 'Please make sure you visit our', 'opstore' ), esc_url( admin_url( 'themes.php?page=welcome-page' ) ), esc_html__( 'Welcome Page', 'opstore' ), esc_html__( 'to get started with Opstore.', 'opstore' ) );
			?>
			</p>
			<p><a class="button button-primary" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page' )) ?>"><?php esc_html_e( 'Lets Get Started', 'opstore' ); ?></a></p>
			</div>
			<?php
			...
			public function welcome_styles_and_scripts() {
				wp_enqueue_style( 'opstore' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/css/welcome.css' );
				wp_enqueue_script( 'opstore' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array( 'jquery' ) );

				wp_localize_script( 'opstore' . '-welcome-screen', 'operationWelcomeObject', array(
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'opstore' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'opstore') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'opstore');
				}
			...
			public function all_required_plugins_installed() {
		  	...
		  	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=' . 'opstore' . '-welcome&section=actions_required'),'remote-file-installation');
	      	...
	      	public function admin_sidebar_contents(){ ?>
	      		<div class="operation-admin-side-wrapper">
	      			...
	      			<div class="operation-side-box-wrapper operation-welcome-box-white">
						<div class="box-header op-box-header"><?php esc_html_e('Buy Pro','opstore'); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('With Opstore Pro publish your online store within a seconds. ','opstore'); ?></p>
							<a href="<?php echo esc_url($this->strings['pro_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('Details','opstore'); ?></a>
						</div>
					</div>

					<div class="operation-side-box-wrapper operation-welcome-box-white">
						<div class="box-header op-box-header"><?php esc_html_e('Theme Documentation','opstore'); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('We have online documentation to make your configuration process easy, go through the documentation to get more familiar with theme','opstore'); ?></p>
							<a href="<?php echo esc_url($this->strings['doc_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('Read Now','opstore'); ?></a>
						</div>
					</div>

					<div class="operation-side-box-wrapper operation-welcome-box-white">
						<div class="box-header op-box-header"><?php printf(esc_html__('Loving %1$s ? ','opstore'),$this->theme_name); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('If you are enjoying theme,spread your love and rate our theme.','opstore'); ?></p>
							<a href="https://wordpress.org/support/theme/<?php echo esc_attr($this->theme_name)?>/reviews/#new-post" class="button button-primary" target="_blank"><?php esc_html_e('Rate Now','opstore'); ?></a>
						</div>
					</div>
